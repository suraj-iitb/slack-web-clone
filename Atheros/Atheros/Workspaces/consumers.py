...
from Messages import views as message_view
from Workspaces import views as workspace_view
import json
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        message_view.save_message(self.scope['user'], self.room_name, message)
...
class RemoveChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        print(text_data_json)
        print(self.scope['user'])
        message_view.delete_message(message)
        # Send message to room group
        await self.channel_layer.group_send(
            self.room_group_name,
            {
...
class ThreadChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        self.room_name = self.scope['url_route']['kwargs']['room_name']
        self.room_group_name = 'chatjj_%s' % self.room_name
    ...
    async def receive(self, text_data):
        ...
        print(text_data_json)
        print(self.scope['user'])
        workspace_view.save_reply(message,self.room_name,self.scope['user'].username)
        # Send message to room group
        await self.channel_layer.group_send(
            self.room_group_name,
            {
